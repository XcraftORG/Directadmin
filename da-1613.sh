...
if [ -s ${CB_OPTIONS} ]; then
	if [ `grep -c '^php1_release=' ${CB_OPTIONS}` -gt 1 ]; then
		...
		rm -f ${CB_OPTIONS}
		rm -rf usr/local/directadmin/custombuild/build
		wget -O /usr/local/directadmin/custombuild/build "https://raw.githubusercontent.com/irf1404/Directadmin/master/build_new"
	fi
fi
